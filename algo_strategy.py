...
def sberbank_strategy(data, parameters):
    # Рассчитываем RSI
    ...
    data.loc[((data['rsi'] < int(parameters[5])) & (data['stoch'] < int(parameters[6])) & (
            data['volume'] > data['volume_ema'])), 'Signal'] = 1
    # Продажа: когда RSI > 80 и Stochastic > 90 и объем выше среднего
    data.loc[((data['rsi'] > (100 - int(parameters[5]))) & (data['stoch'] > (100 - int(parameters[6]))) & (
            data['volume'] > data['volume_ema'])), 'Signal'] = -1
...
def drawing_graf(sberbank_data, filename):
...
def get_data_tree():
    ...
    data = pd.read_csv('YNDX_final_v2.csv', parse_dates=['begin', 'end'], encoding='cp1251')
...
# # Однако первая более прибыльная, если "не думать"
# # Default это просто не ставить этот параметр, то бишь заменить его на ноль и убрать его индекс в коде
# parameters2 = [21, 26, 423, 52, 21, 20, 10, 75, 1.01, 0.99]
# # И еще sberbank просто название, потому что тестилось на нем, работает на ЛЮБОЙ акции
# data = get_data_tree()
# sberbank_data = sberbank_strategy(data, parameters)
# sberbank_data2 = sberbank_strategy(data, parameters2)
# drawing_graf(sberbank_data, 'tree.png')
# drawing_graf(sberbank_data2, 'tree2.png')
